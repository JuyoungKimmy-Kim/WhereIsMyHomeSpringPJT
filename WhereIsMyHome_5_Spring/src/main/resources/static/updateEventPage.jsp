...
<body>
	<div style="display: none;" id="event_id"><%= key %></div>
	<div class="container-fluid" style="width: 50%; padding-top: 10rem">
        <form class="row g-3">
            ...
            <div class="col-12">
              <button type="submit" id="btnUpdateEvent" class="btn btn-primary">이벤트 수정</button>
            </div>
          </form>    
    </div>
...
window.onload = function() {
	document.querySelector("#btnUpdateEvent").onclick = function(e) {
...
async function updateEvent() {
    ...
    let key = document.querySelector("#event_id").innerText;
    ...
    let response = await fetch( "<%= contextPath%>/event/update", fetchOptions);
    let data = await response.json();
    
    if( data.result == "success" ){
    	
    	alertify.alert("이벤트 수정 완료", function(){
